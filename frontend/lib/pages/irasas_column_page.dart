...
class _IrasasColumnPageState extends State<IrasasColumnPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final rowIrasai = (results[0]).map((e) => e as Map<String, dynamic>).toList();
      final rows = (results[1]).map((e) => e as Map<String, dynamic>).toList();
      final templates = (results[2]).map((e) => e as Map<String, dynamic>).toList();
      final values = (results[3]).map((e) => e as Map<String, dynamic>).toList();
      final headers = (results[4]).map((e) => e as Map<String, dynamic>).toList();
      final rowValues = (results[5]).map((e) => e as Map<String, dynamic>).toList();
      ...
      setState(() {
        ...
        _userNameById = {
          for (final u in (naudotojai).map((e) => Map<String, dynamic>.from(e as Map)))
            ((u['id'] ?? u['Id']).toString()): ('${((u['vardas'] ?? u['Vardas'] ?? '') as String).trim()} ${((u['pavarde'] ?? u['Pavarde'] ?? '') as String).trim()}').trim()
        };
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Map<String, dynamic>? _getRowValueForRowIrasas(Map<String, dynamic> r) {
    ...
    final found = _rowValues.firstWhere(
      (v) => (v['rowIrasasId'] ?? v['row_irasas_id'] ?? v['RowIrasasId']) == rowIrasasId && (v['rowId'] ?? v['row_id'] ?? v['RowId']) == rowId,
  ...
  Future<void> _saveRowValue(Map<String, dynamic> r, String value) async {
    try {
      ...
      if (!mounted) return;
      await _load();
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Eilutės būsena išsaugota')));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Klaida įrašant eilutės būseną: $e')));
    }
  }
  ...
  Future<void> _saveValue({
    ...
    final isArray = tpl['isArray'] ?? tpl['is_array'] ?? tpl['IsArray'] ?? false;
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Neteisingas skaičiaus formatas')));
      return;
    }
    ...
    try {
      ...
      _editing.remove(key);
      await _load();
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Įrašyta')));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Klaida įrašant reikšmę: $e')));
    }
  }

  Future<void> _saveArrayValue({required String key, required Map<String, dynamic> r, required Map<String, dynamic> tpl, required int arrayLen}) async {
    final values = <num>[];
    ...
    final existing = _columnValues.firstWhere(
      (v) => (v['rowIrasasId'] ?? v['row_irasas_id'] ?? v['RowIrasasId']) == rowIrasasId && (v['columnTemplateId'] ?? v['column_template_id'] ?? v['ColumnTemplateId']) == columnTemplateId,
      orElse: () => <String, dynamic>{},
    );
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Klaida įrašant masyvą: $e')));
    }
  }
  ...
  String? _headerTextForTemplate(Map<String, dynamic> template) {
    final hid = template['headerId'] ?? template['header_id'] ?? template['HeaderId'];
                ...
                child: _rowIrasai.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (ctx, index) {
                          ...
                          final rowId = r['rowId'] ?? r['rowid'] ?? r['row_id'];
                          final templates = _columnTemplates.where((t) => (t['rowId'] ?? t['row_id'] ?? t['RowId']) == rowId).toList();
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  if ((rowDef['controlMethods'] ?? '') is String && (rowDef['controlMethods'] ?? '').toString().isNotEmpty) ...[
                                    Text('Kontrolės metodai: ${(rowDef['controlMethods'] ?? '') as String}', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant)),
                                    const SizedBox(height: 8),
                                  ],
                                  Builder(builder: (ctx) {
                                    final rv = _getRowValueForRowIrasas(r);
                                    final current = (rv == null) ? 'empty' : (rv['value'] ?? rv['Value'] ?? 'empty');
                                    final completedRaw = rv == null ? null : (rv['completed_at'] ?? rv['completedAt'] ?? rv['CompletedAt']);
                                    final completedBy = rv == null ? null : (rv['completed_by_user_id'] ?? rv['completedByUserId'] ?? rv['CompletedByUserId'])?.toString();
                                    String? compText;
                                    if (completedRaw != null) {
                                      final dt = DateTime.tryParse(completedRaw.toString())?.toLocal();
                                      ...
                                      final uname = completedBy == null ? '' : (_userNameById[completedBy] ?? completedBy);
                                      compText = 'Pabaigtas: $dateStr • $uname';
                                    }

                                    return Row(
                                      children: [
                                        DropdownButton<String>(
                                          value: current is String ? current : current.toString(),
                                          items: const [
                                            DropdownMenuItem(value: 'empty', child: Text('empty')),
                                            DropdownMenuItem(value: 'compliant', child: Text('compliant')),
                                            DropdownMenuItem(value: 'non-compliant', child: Text('non-compliant')),
                                        ...
                                        if (compText != null) Expanded(child: Text(compText, style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant))),
                                      ],
                                    );
                                  }),
                                    ...
                                    SingleChildScrollView(
                                      ...
                                      child: Builder(builder: (ctx) {
                                        ...
                                        for (var tpl in templates) {
                                          final hid = tpl['headerId'] ?? tpl['header_id'] ?? tpl['HeaderId'];
                                        ...
                                        Widget buildTplWidget(Map<String, dynamic> tpl) {
                                          ...
                                          final isArray = tpl['isArray'] ?? tpl['is_array'] ?? tpl['IsArray'] ?? false;
                                          final arrayLen = tpl['arrayLength'] ?? tpl['array_length'] ?? tpl['ArrayLength'] ?? 0;
                                          final tplId = (tpl['id'] ?? tpl['Id']).toString();
                                          final rId = (r['id'] ?? r['Id']).toString();
                                          final key = '${rId}_$tplId';

                                          String? _completedText() {
                                            if (valueMap == null) return null;
                                            final raw = valueMap['completed_at'] ?? valueMap['completedAt'] ?? valueMap['CompletedAt'];
                                            if (raw == null) return null;
                                            final dt = DateTime.tryParse(raw.toString())?.toLocal();
                                            final dateStr = dt == null ? '' : '${dt.year.toString().padLeft(4, '0')}-${dt.month.toString().padLeft(2, '0')}-${dt.day.toString().padLeft(2, '0')} ${dt.hour.toString().padLeft(2, '0')}:${dt.minute.toString().padLeft(2, '0')}';
                                            final uid = (valueMap['completed_by_user_id'] ?? valueMap['completedByUserId'] ?? valueMap['CompletedByUserId'])?.toString();
                                            final uname = uid == null ? '' : (_userNameById[uid] ?? uid);
                                            return 'Pabaigtas: $dateStr • $uname';
                                          }

                                          if (isArray == true || isArray == 1) {
                                            if (arrayLen is int && arrayLen > 0) {
                                              final existingList = (valueMap == null)
                                                  ? List<String>.filled(arrayLen, '')
                                                  : ((valueMap['arrayValue'] ?? valueMap['array_value'] ?? valueMap['arrayValue']) is List
                                                      ? List<String>.from(((valueMap['arrayValue'] ?? valueMap['array_value'] ?? valueMap['arrayValue']) as List).map((e) => e?.toString() ?? ''))
                                                      : List<String>.filled(arrayLen, ''));
                                              for (var i = 0; i < arrayLen; i++) {
                                                final k = '${key}_$i';
                                                _controllers.putIfAbsent(k, () => TextEditingController(text: i < existingList.length ? existingList[i] : ''));
                                              }
                                              return Column(
                                                crossAxisAlignment: CrossAxisAlignment.start,
                                                children: [
                                                  Text(tpl['description'] ?? tpl['Description'] ?? 'Stulpelis #$tplId', style: const TextStyle(fontWeight: FontWeight.w600)),
                                                  const SizedBox(height: 6),
                                                  // Use Wrap so long arrays flow to multiple rows instead of causing layout issues
                                                  Wrap(
                                                    spacing: 8,
                                                    runSpacing: 8,
                                                    children: List.generate(arrayLen, (i) {
                                                      final k = '${key}_$i';
                                                      ...
                                                      return SizedBox(
                                                        ...
                                                        child: Padding(
                                                          ...
                                                          child: TextField(
                                                            ...
                                                            onSubmitted: (_) async {
                                                              await _saveArrayValue(key: key, r: r, tpl: tpl, arrayLen: arrayLen);
                                                            },
                                                          ),
                                                        ),
                                                      );
                                                  ...
                                                  const SizedBox(height: 6),
                                                  ElevatedButton(onPressed: () async => await _saveArrayValue(key: key, r: r, tpl: tpl, arrayLen: arrayLen), child: const Text('Išsaugoti')),
                                                  const SizedBox(height: 6),
                                                  if (_completedText() != null) Text(_completedText()!, style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant, fontSize: 12)),
                                                ],
                                              );
                                            }
                                            final initialText = valueMap == null
                                                ? ''
                                                : ((valueMap['arrayValue'] ?? valueMap['array_value'] ?? valueMap['arrayValue']) is List
                                                    ? (valueMap['arrayValue'] ?? valueMap['array_value'] ?? valueMap['arrayValue']).join(', '
                                                    )
                                                    : '');
                                            final controller = _controllers.putIfAbsent(key, () => TextEditingController(text: initialText));
                                            return Column(
                                              crossAxisAlignment: CrossAxisAlignment.start,
                                              children: [
                                                Text(tpl['description'] ?? tpl['Description'] ?? 'Stulpelis #$tplId', style: const TextStyle(fontWeight: FontWeight.w600)),
                                                  ...
                                                  children: [
                                                    SizedBox(
                                                      ...
                                                      child: TextField(
                                                        ...
                                                        onSubmitted: (_) => _saveValue(key: key, r: r, tpl: tpl),
                                                      ),
                                                    ),
                                                    IconButton(icon: const Icon(Icons.check), onPressed: () => _saveValue(key: key, r: r, tpl: tpl)),
                                                  ],
                                                ),
                                                const SizedBox(height: 6),
                                                if (_completedText() != null) Text(_completedText()!, style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant, fontSize: 12)),
                                          ...
                                          final initialText = valueMap == null ? '' : ((valueMap['singleValue'] ?? valueMap['single_value'] ?? valueMap['singleValue'])?.toString() ?? '');
                                          final controller = _controllers.putIfAbsent(key, () => TextEditingController(text: initialText));
                                          return Column(
                                            crossAxisAlignment: CrossAxisAlignment.start,
                                            children: [
                                        ...
                                        return Row(
                                          ...
                                          children: groupKeys.map<Widget>((k) {
                                            ...
                                            final headerText = k == null ? null : (group.firstWhere((t) => true)['headerId'] == null ? null : _headerTextForTemplate(group.first));
                                            return Container(
                                              margin: const EdgeInsets.symmetric(horizontal: 8, vertical: 6),
                                              decoration: BoxDecoration(
                                                border: Border.all(color: Theme.of(context).colorScheme.primary.withOpacity(0.22), width: 2.0),
                                                borderRadius: BorderRadius.circular(10),
                                                color: Theme.of(context).colorScheme.surface,
                                                boxShadow: [
                                                  BoxShadow(color: Theme.of(context).shadowColor.withOpacity(0.06), blurRadius: 8, offset: const Offset(0, 3)),
                                                ],
                                              ),
                                              child: Column(
                                                crossAxisAlignment: CrossAxisAlignment.start,
                                                children: [
                                                  ...
                                                  // Templates laid out horizontally beneath the banner
                                                  Padding(
                                                    padding: const EdgeInsets.all(12),
                                                    child: SingleChildScrollView(
                                                      ...
                                                      child: Row(
                                                        crossAxisAlignment: CrossAxisAlignment.start,
                                                        children: group.map<Widget>((tpl) {
                                                          return Padding(
                                                            padding: const EdgeInsets.only(right: 18.0),
                                                            child: ConstrainedBox(
                                                              constraints: const BoxConstraints(maxWidth: 360),
                                                              child: buildTplWidget(tpl),
                                                            ),
                                                          );
                                                        }).toList(),
                                                      ),
